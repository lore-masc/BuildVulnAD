...
$Global:SPNs = @('HOST/mssqlserver','DNS/httpserver','LDAP/exserver');
$Global:CreatedUsers = @();
$Global:CreatedPasswords = @();
$Global:Assets = @();
$Global:KerberoastableUsers = @()
$Global:ASREPRoastableUsers = @()
$Global:AllObjects = @();
$Global:Domain = "";
#Strings 
$Global:Spacing = "`t"
...
function ShowBanner {
    ...
    $banner+= $Global:Spacing + 'VULN AD - Build a vulnerable PATH in your Active Directory'
    $banner+= $Global:Spacing + ''                                                  
    $banner+= $Global:Spacing + 'By lorenki @lore-masc'
...
function VulnAD-AddADUser {
    ...
    for ($i=1; $i -le $limit; $i=$i+1 ) {
        ...
        $Global:CreatedPasswords += $generated_password;
    }
...
function VulnAD-Kerberoasting {	
	$perc = [math]::ceiling($Global:CreatedUsers.Count/100*20)
    for ($i=0; $i -le (Get-Random -Maximum $perc); $i=$i+1 ) {
    	$random_id = Get-Random -Maximum $Global:CreatedUsers.Count
        $user = $Global:CreatedUsers[$random_id]

        $selected_spn = (VulnAD-GetRandom -InputList $Global:SPNs)
        $password = VulnAD-GetRandom -InputList $Global:BadPasswords;
        $Global:KerberoastableUsers += $user
        Write-Info "Kerberoasting $user"

        net user $user $password | Out-Null
        $Global:CreatedPasswords[$random_id] = $password;
        setspn $user -s $selected_spn | Out-Null
    }
}
function VulnAD-ASREPRoasting {
    ...
    for ($i=0; $i -le (Get-Random -Maximum $perc); $i=$i+1 ) {
        ...
        $Global:ASREPRoastableUsers += $user
        $Global:CreatedPasswords[$random_id] = $password;
        Write-Info "AS-REPRoasting $user"
    }
}
...
function VulnAD-DefaultPassword {
    for ($i=1; $i -le (Get-Random -Maximum 5); $i=$i+1 ) {
        ...
        Set-AdAccountPassword -Identity $user -Reset -NewPassword (ConvertTo-SecureString $default_password -AsPlainText -Force)
        Set-ADUser $user -Description "New User ,DefaultPassword"
        #Set-AdUser $user -ChangePasswordAtLogon $true
        $Global:CreatedPasswords[$random_id] = $default_password;
        Write-Info "Default Password : $user"
    }
}
...
function VulnAD-AddRemoteDesktopUsers {
	[CmdletBinding()]
        param(
            [Parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
...
function VulnAD-UnquotedService {
	[CmdletBinding()]
        param(
        	[Parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
...
function VulnAD-LocalAdmin {
    ...
    Invoke-Command -ComputerName $Hostname -Credential $Credential -ScriptBlock {
	    cmd /c net localgroup "Administrators" $using:User /add | Out-Null
	}
}
function VulnAD-AddLSASSCred {
	[CmdletBinding()]
        param(
        	[Parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
	...
	# Start and discconnect a RDP session
	cmdkey /generic:$Hostname /user:$User /pass:$Password 
	mstsc /v:$Hostname
	Write-Info "Waiting after RDP user logon. Please, not disconnect the user!"
	cmd /c pause
	$id=Invoke-Command -ComputerName $Hostname -ScriptBlock {(Get-Process rdpclip | Select-Object SessionId).sessionid}
	Start-Sleep 10
	taskkill /F /IM mstsc.exe
}
function VulnAD-ConstrainedUser {
	[CmdletBinding()]
        param(
        	[Parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
...
function VulnAD-MSSQL{
    ...
    $netbiosName=$((Get-ADDomain -Identity (Get-WmiObject Win32_ComputerSystem).Domain).NetBIOSName)

	Copy-Item .\SQL2022-SSEI-Dev.exe \\$Hostname\c$\

	while ($true) {
	    $testConnection = Test-Connection -ComputerName 1.1.1.1 -Source $Hostname -Count 1 -ErrorVariable errmsg 2>$null
	    if ($testconnection -ne $null) {
	       $conn=0
	       Write-Info "Install MSSQL server..."
	       Invoke-Command -ComputerName $Hostname -ScriptBlock {
	           cmd /c C:\SQL2022-SSEI-Dev.exe /Q /IACCEPTSQLSERVERLICENSETERMS /ACTION="install" 
	       }
	       Write-Good "MSSQL server installed!"
	   } else {
	        $conn=1
	   }

	   if ($conn -eq 1) {
	    Write-Bad "Asset $Hostname not connected. Retry in 10s..."
	    Start-Sleep 10
	   } else {
	    break
	   }
...
function VulnAD-CreateExclusion {
	[CmdletBinding()]
        param(
            [Parameter(Mandatory=$true)]
            [ValidateNotNullOrEmpty()]
    ...
    $path = 'C:\temp\'
    Invoke-Command -ComputerName $Hostname -ScriptBlock {
  	  New-Item -Path $using:path -ItemType Directory | Out-Null
...
if ($installState -eq "Installed") {
  	...
  	Write-Output "Active Directory has been already installed."
}
else {
...
net user $attacker_username Password% /add | Out-Null

# Shuffle assets and append the DC as last node
$randomized_assets = @($Global:Assets | Sort-Object { Get-Random } -Unique)
$randomized_assets += $env:computername | Out-String
$Global:Spacing + ''

for ($i=0; $i -lt $randomized_assets.Count-1; $i=$i+1) {
	# Select correct hostname
	$hostname = $randomized_assets[$i]
	...
	if ( $vuln_type -eq 0 -and $should_be_admin -eq $false ) {
    	...
    	switch ( $type )
    	{
    		...
    		1 {
    			# Service file
    			VulnAD-ServiceFile -Hostname $hostname -Credential $admin -Path $scriptPath -ServiceName $serviceName
    ...
    if ( $vuln_type -eq 1 ) { 
    	...
    	# Get a REPRoasting user
    	if ( $user_type -eq 1 ) {
    		$user = (VulnAD-GetRandom -InputList $Global:ASREPRoastableUsers)
    		Write-Good "[$i] - $user can be target for REPRoasting attack"
    	}

    	# Get a Kerberoasting user
    	if ( $user_type -eq 2 ) {
    		$user = (VulnAD-GetRandom -InputList $Global:KerberoastableUsers)
    		Write-Good "[$i] - $user can be target for Kerberoasting attack"
    	}
    	
    	# Get the next node
    	$linked_hostname = $randomized_assets[$i+1].Trim()
    	VulnAD-LocalAdmin -Hostname $linked_hostname -Credential $admin -User $user
    	Write-Good "[$i] - Local admin $user created on $linked_hostname"
    }
    if ( $vuln_type -eq 2 ) {
    	...
    	# Get LSASS user
    	if ( $user_type -eq 0 ) {   
    		# Create local admin a random domain user 
    		$random_id = Get-Random -Maximum $UsersLimit
    		$user = $Global:CreatedUsers[$random_id]
    		$pass = $Global:CreatedPasswords[$random_id]
    		
    		VulnAD-LocalAdmin -Hostname $hostname -Credential $admin -User $user
    		Write-Good "[$i] - Local admin $user created on $hostname"

    		# Put selected user credentials inside LSASS
            VulnAD-AddRemoteDesktopUsers -Hostname $hostname -User $user
    		VulnAD-AddLSASSCred -Hostname $hostname -User "$($config.domain.netbiosName)\$($user)" -Password $pass
    		Write-Good "[$i] - Local admin $user credentials are in $hostname LSASS process ($pass)"
    	}
    	...
    	VulnAD-ConstrainedUser -User $user -Hostname $linked_hostname
    ...
    if ( $vuln_type -eq 3 ) {
    	...
    	# Create target user as local administrator
    	$random_id = Get-Random -Maximum $UsersLimit
    	$target_user = $Global:CreatedUsers[$random_id]
    	$target_pass = $Global:CreatedPasswords[$random_id]
    ...
    if ( $vuln_type -eq 4 ) {
    	...
    	VulnAD-DCSync -Username $user
    	Write-Good "[$i] - $user can be used for DCSync attack"
    }
    if ( $vuln_type -eq 5 ) {
    	# LSASS dump
		...
		# User inside LSASS process is also local admin to the next host
		VulnAD-LocalAdmin -Hostname $($randomized_assets[$i+1].Trim()) -Credential $admin -User $user
		Write-Good "[$i] - Local admin $user created on $($randomized_assets[$i+1].Trim())"
    ...
    if ( $vuln_type -eq 6 ) { 
    	...
    	VulnAD-UnconstrainedComputer -ComputerAccount $hostname
    	Write-Good "[$i] - Unconstrained delegation created for $hostname"
    }
    if ( $vuln_type -eq 7 ) {
    	...
    	VulnAD-ConstrainedComputer -ComputerAccount $hostname -Hostname $linked_hostname
    	Write-Good "[$i] - Constrained delegation created for $hostname$ on $linked_hostname"
    }
}
...
# Select first asset
$hostname = $randomized_assets[0]

# Create exclusion on first asset (optional)
if ($config.startExclusion -eq 1) {
	VulnAD-CreateExclusion -Hostname $hostname
	Write-Good "Add exclusion directory on $hostname"
}

VulnAD-AddRemoteDesktopUsers -Hostname $hostname -User $attacker_username
# Create attacker user
Write-Good "You can start with the following domain user connecting on $($hostname):"
Write-Info "Username: $($config.domain.netbiosName)\$($attacker_username)"
